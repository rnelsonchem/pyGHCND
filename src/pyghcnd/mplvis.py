...
def plot_temp(ghcnd, use_year=None, trends=True, smooth=15, 
    ...
    for dcat, color in [('TMAX', 'r'), ('TMIN', 'b')]:
        ...
        if trends:
            ...
            final_year = ghcnd.now.year - ghcnd.start_date
    ...
    if trends:
        ...
        leg_str.extend([f'linear est. {ghcnd.start_date}', 
                f'linear est. {ghcnd.now.year}',])
    ...
    plt.axvline(datetime(2000, ghcnd.now.month, ghcnd.now.day),
            c='0.1', lw=0.5)
    ...
    if save:
        plt.savefig(ghcnd.folder / f'yearly_plot.png', 
                dpi=dpi)
    if show:
        plt.show()
    plt.close()

def plot_prcp(ghcnd, use_year=None, ptype='PRCP', n_missing=10, 
            show=True, save=True, dpi=300):
    ...
    if save:
        plt.savefig(ghcnd.folder / f'precip_plot.png', 
                dpi=dpi)
    if show:
        plt.show()
    plt.close()

def plot_daily_temp(ghcnd, month, day, temp_type='both', p=0.05, 
            show=True, save=True, dpi=300):
    ...
    for cat, top, bottom in cats:
        mask = (ghcnd.raw.index.month == month) & (ghcnd.raw.index.day == day)
        data = ghcnd.raw.loc[mask, cat]
        years = data.index.year

        ydiff = ghcnd.raw.loc[mask, 'yeardiff']
        slope = ghcnd.stats.loc[(month, day), (cat, 'slope')]
        icept = ghcnd.stats.loc[(month, day), (cat, 'icept')]
        line = slope*ydiff + icept
        
        pval = ghcnd.stats.loc[(month, day), (cat, 'p_slope')]
    ...
    if save:
        plt.savefig(ghcnd.folder / f'daily_temp_plot_{temp_type}.png', 
                dpi=dpi)
    if show:
        plt.show()
    plt.close()

def plot_daily_trends(ghcnd, p=0.05, show=True, save=True, dpi=300):
    fig = plt.figure(figsize=(8,4))
    ...
    for cat in cats:
        ...
        slopes = ghcnd.stats.loc[:, (cat, 'slope')]
        pvals = ghcnd.stats.loc[:, (cat, 'p_slope')]
    ...
    if save:
        plt.savefig(ghcnd.folder / 'daily_trends.png', dpi=dpi)
    if show:
        plt.show()
    plt.close()

def plot_temp_diffs(ghcnd, ndays=90, show=True, save=True, dpi=300):
    fig = plt.figure(figsize=(8,4))
    top = plt.subplot(211)
    bot = plt.subplot(212)

    for cat, ax in [('TMAX', top), ('TMIN', bot)]:
        recent = ghcnd.raw[[cat]].iloc[-ndays:].reset_index()
        dates = recent['date']
        recent = recent.set_index([recent.date.dt.month, recent.date.dt.day])\
                        .rename_axis(index=['month', 'day'])[cat]
        index = recent.index

        mean = ghcnd.stats.loc[index, (cat, 'mean')]
        std = ghcnd.stats.loc[index, (cat, 'std')]
        mint = ghcnd.stats.loc[index, (cat, 'min')] - mean
        maxt = ghcnd.stats.loc[index, (cat, 'max')] - mean
        resid = (recent - mean)
    ...
    if save:
        plt.savefig(ghcnd.folder / 'temp_diffs.png', dpi=dpi)
    if show:
        plt.show()
    plt.close()
